...
public interface StylerBootstrap {
     ...
     * @implSpec This method is only called once per each instance of the parser & styler.
     * It must not assume that the first invocation is the {@link TextStyler#INSTANCE} parser &amp; styler,
     * and that each parser &amp; styler given to the bootstrap must be extended in the exact
     * same way each time when being implemented for use by {@code fabric.mod.json}.
